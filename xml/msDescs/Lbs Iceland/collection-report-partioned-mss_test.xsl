    ...
    <html>
      ...
      <body>  
       ...
       <!-- <p>Number of mss containing bsh: <xsl:value-of select="count($bshDocuments)"></xsl:value-of></p>-->
        ...
        <p>secound count of fas items based on @ref: <xsl:value-of select="count($fasDocuments//title[@ref])"></xsl:value-of></p>
        
        <p>Report mss and mss parts with similar layout:
        
        Manuscripts with 23 to 36 lines:
          <xsl:for-each select="$fasDocuments//layout[@writtenLines='23 36']">
            <xsl:value-of select="ancestor::msDesc//idno"></xsl:value-of>
          </xsl:for-each>
        </p>
    
   <!-- <p>Distinct values of personal names (key of name element with type="person"):
